...
def imageProcessing(photo_path):
    img = cv2.imread(photo_path)
    imgray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ret, thresh = cv2.threshold(imgray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    contours, hierarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    n1, n2, n3 = contours
    n1_green = getAverageRGB(img, n1)[1]
    n2_green = getAverageRGB(img, n2)[1]
    n3_green = getAverageRGB(img, n3)[1]
    hole_green = [n1_green, n2_green, n3_green]
    hole1_green = min(hole_green)  # hole1_green is minimum number
    hole2_green = max(hole_green)  # hole2_green is maximum numbermin
    hole_green.remove(hole1_green)
    hole_green.remove(hole2_green)
    hole3_green = hole_green[0]  # hole3_green is median number
    log.info('Detection area of hole1 average Green value : {}'.format(hole1_green))
    log.info('Detection area of hole2 average Green value : {}'.format(hole2_green))
    log.info('Detection area of hole3 average Green value : {}'.format(hole3_green))
    if abs(hole3_green - hole1_green) < 10:
        log.info('The sample to be tested is negative !')
...
def getWeather(city):
    ...
    log.info('Nanjing {} temperature : {}'.format(strftime('%Y-%m-%d %H:%M:%S', localtime()), str(temp)))
    return temp
...
def calculate(temp, RGB_2, RGB_3):  # RBG_2 is hole2 G value, RGB is hole3 G value
    ...
    log.info("PH : {}".format(PH))
    if PH >= 8.2 or PH < 5.8:
        log.warning("Test limit exceeded !")
    return x, PH